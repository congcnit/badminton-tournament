...
export default function DashboardClient({
  ...
  const { players, teams, rounds, hasHydrated, hydrate } = useTournamentStore();
  ...
  const assignedPercent = totalPlayers > 0 ? (assignedPlayers / totalPlayers) * 100 : 0;
  const teamsReady = useMemo(
    () => teams.filter((t) => t.players.length === 8).length,
    [teams]
  );
