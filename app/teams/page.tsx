...
export default function TeamsPage() {
  ...
  // Sort function for players by gender (Female first) and then by name
  const sortPlayers = (a: typeof unassignedPlayers[0], b: typeof unassignedPlayers[0]) => {
    // First sort by gender (F comes before M)
  ...
  const unassignedPlayersByLevel: Record<Level, typeof unassignedPlayers> = {
    'Nguyên Anh': unassignedPlayers.filter((p) => p.level === 'Nguyên Anh').sort(sortPlayers),
    'Kết Đan': unassignedPlayers.filter((p) => p.level === 'Kết Đan').sort(sortPlayers),
    'Trúc Cơ': unassignedPlayers.filter((p) => p.level === 'Trúc Cơ').sort(sortPlayers),
    'Luyện Khí Kỳ': unassignedPlayers.filter((p) => p.level === 'Luyện Khí Kỳ').sort(sortPlayers),
  };
